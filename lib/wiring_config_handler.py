...
class WiringConfigHandler(ZynthianConfigHandler):
    PROFILES_DIRECTORY = "{}/wiring-profiles".format(
    ...
    def prepare(self):
        super().prepare()
        self.current_custom_profile = os.environ.get(
    ...
    def get(self, errors=None):
        ...
        config['ZYNCODER_MULTITIMBRAL_DEFAULT'] = {
            'type': 'select',
            'title': "MIDI Device Default Mode",
            'value': os.environ.get('ZYNCODER_MULTITIMBRAL_DEFAULT', ""),
            'options': ['0', '1'],
            'option_labels': {
                        '0': '⇥ Active Channel', #\u21e5
                        '1': '⇶ Multi Timbral' #\u21f6
        ...
        if self.n_custom_switches > 0:
            ...
            for i in range(self.n_custom_switches):
                ...
                cuia_name, cuia_param = self.get_cuia(
                    base_name + '__UI_ALT_PUSH')
    ...
    def post(self):
        ...
        if command == 'REFRESH':
            errors = None
            self.current_custom_profile = self.get_argument(
                'ZYNTHIAN_WIRING_LAYOUT_CUSTOM_PROFILE', '')
            logging.debug("CURRENT CUSTOM PROFILE => {}".format(
                self.current_custom_profile))
            ...
            fname = self.get_argument(
                'zynthian_wiring_layout_saveas_fname', '')
            ...
            self.current_custom_profile = next(
    ...
    def get_request_data(self):
        ...
        for i in range(64):
            ...
            for k in ("PUSH", "SHORT", "BOLD", "LONG", "ALT_PUSH", "ALT_SHORT", "ALT_BOLD", "ALT_LONG"):
                ...
                try:
                    cuia_str = data[base_subname + '__CUIA_NAME'][0] + \
                        " " + data[base_subname + '__CUIA_PARAM'][0]
            ...
            if k.startswith("ZYNTHIAN_WIRING_"):
                ...
                for ivn in ignore_varnames:
                    if kk.startswith(ivn):
                        ignore = True
                        break
            if 'ZYNCODER_MULTITIMBRAL_DEFAULT' in k:
                ignore = False
                logging.debug("Detected {}: {} <> {}".format(
